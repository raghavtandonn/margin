...
/**
 * Star glyphs, as markup, at one of the sizes the stylesheet defines.
 *
 * Nine templates call this — shelf, desk, book, profile, criticism, reviews
 * and the ticket — with `<%-` because it returns an element and not a string:
 * the glyphs carry a colour and a size that the surrounding body text does
 * not, and "4 of 5 stars" belongs on the element rather than in the glyph run
 * where a screen reader would spell out each character.
 *
 * The size is a CLASS and never a style attribute. A nonce can be attached to
 * a <style> element but never to a style="" attribute, so the strict CSP has
 * no way to permit one — the same rule h.rule() exists for. Only the four
 * sizes margin.css actually defines are honoured; anything else inherits
 * rather than emitting a class that resolves to nothing.
 */
const STAR_SIZES = new Set([11, 12, 13, 16]);

export function stars(value, { size = null } = {}) {
  const glyphs = starGlyphs(value);
  if (!glyphs) return '';            // unrated reads as absent, never as zero
  const n = Number(size);
  const sized = STAR_SIZES.has(n) ? ` stars-${n}` : '';
  return `<span class="stars${sized}" aria-label="${esc(starLabel(value))}">${glyphs}</span>`;
}
